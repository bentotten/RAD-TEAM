...
import warnings
from rl_tools.mpi_tools import proc_id, mpi_statistics_scalar
from rl_tools.serialization_utils import convert_json
...
class Logger:
    ...
    def save_config(self, config, text='', quiet=False):
        """
...
class EpochLogger(Logger):
    ...
    def log_tabular(self, key, val=None, with_min_and_max=False, 
                    average_only=False, sum_only=False, rate_only=False):
        """
        ...
        if val is not None:
            ...
            #if not v:
            #    v = [0]
            #    print(f'No val in V!!: {key}')
                
            #else:
            vals = np.concatenate(v) if isinstance(v[0], np.ndarray) and len(v[0].shape)>0 else v   
            stats = mpi_statistics_scalar(vals, with_min_and_max=with_min_and_max, sum_only=sum_only)

            if not(rate_only):
                super().log_tabular(key if (average_only or sum_only) else 'Average' + key, stats[0])
            if not(average_only) and not(sum_only):
