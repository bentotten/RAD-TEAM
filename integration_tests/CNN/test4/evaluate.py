        ...
        if ALL_ACKWARGS_SAVED:
            for arg in actor_critic_args:
                if arg == "environment_scale":
                    assert actor_critic_args[arg] == pytest.approx(
                        original_configs[arg], rel=1e4
                    ), f"Agent argument mismatch: {arg}.\nCurrent: {actor_critic_args[arg]}; Model: {original_configs[arg]}"

                elif (
                    arg != "no_critic" and arg != "GlobalCritic" and arg != "save_path"
                ):
                    if (
        ...
        while run_counter < self.montecarlo_runs:
            ...
            for id, ac in self.agents.items():
                ...
                hiddens[id] = agent_thoughts[
                    id
                ].hidden  # For RAD-A2C - save latest hiddens for use in next steps.
