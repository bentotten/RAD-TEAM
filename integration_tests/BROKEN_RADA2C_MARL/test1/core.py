...
class RNNModelActorCritic(nn.Module):
    def __init__(
        ...
        # obs_dim_pfgru = int(obs_dim / num_agents)
        obs_dim_pfgru = num_agents * 3 # Only takes coordinates and radiation reading
    ...
    def step(self, obs, hidden=None, id=0, obs_count=1):
        with torch.no_grad():
            ...
            obs_for_pfgru = torch.as_tensor(obs[:, :3], dtype=torch.float32).flatten().unsqueeze(0)
            loc_pred, hidden_part = self.model(obs_for_pfgru, hidden[0]) 
    ...
    def grad_step(self, obs, act, id, hidden=None):
        obs_t = torch.as_tensor(obs, dtype=torch.float32).unsqueeze(1) # Observations from each agent are already flattened into one line for each step
        ...
        obs_for_pfgru = torch.zeros(len(obs), self.num_agents * 3)
        for offset in range(self.num_agents):
            slice_obs = obs[:, (offset*11):(offset*11+3)]
            obs_for_pfgru[:, offset*3:offset*3+3] = slice_obs
            
        obs_for_pfgru = obs_for_pfgru.unsqueeze(1)

        loc_pred = torch.empty((obs_for_pfgru.shape[0], 2))
        hidden_part = hidden[0]
        with torch.no_grad():
            offset = 3 * self.num_agents
            for kk, o in enumerate(obs_for_pfgru):
                loc_pred[kk], hidden_part = self.model(o[:, :offset], hidden_part)
