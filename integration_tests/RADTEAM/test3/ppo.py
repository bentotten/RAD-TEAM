...
def update(ac, buf, optimization, PFGRU, train_pi_iters, train_v_iters, train_pfgru_iters, target_kl, clip_ratio, number_of_agents, id, mode):
    ...
    def compute_loss_pi(
        ...
        :param data: (array) data from PPO buffer. Contains:
            ...
            * adv: (tensor) batch of advantages cooresponding to actions. These are the difference between the expected reward for taking that action
                and the true reward (See: TD Error, GAE).
    ...
    def compute_batched_losses_pi(agent, sample, data, mapstacks_buffer):
        ...
        for index in sample:
            ...
            agent.reset()
            loss_pi, approx_kl, ent, clipfrac = compute_loss_pi(agent=agent, data=data, index=index, map_stack=mapstacks_buffer)
...
def ppo(
    ...
    print(f"Proc id: {proc_id()} -> Starting main training loop in {mode} mode with {number_of_agents} agents!", flush=True)
