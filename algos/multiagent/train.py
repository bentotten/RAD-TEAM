...
class train_PPO:
    ...
    stat_buffers: Dict[int, StatisticStandardization] = field(
    ...
    def __post_init__(self) -> None:
        if self.actor_critic_architecture != "cnn" and (
            self.global_critic_flag or self.GlobalCritic or self.GlobalCriticOptimizer
        ):
        ...
        if self.global_critic_flag:
            prototype = RADCNN_core.CNNBase(
                id=0, **self.ppo_kwargs["actor_critic_args"]
            )
            self.GlobalCritic = RADCNN_core.Critic(
                map_dim=prototype.get_map_dimensions(),
                batches=prototype.get_batch_size(),
                map_count=prototype.get_critic_map_count(),
            )
            self.GlobalCriticOptimizer = Adam(
                self.GlobalCritic.parameters(),
                lr=self.ppo_kwargs["critic_learning_rate"],
            )
        ...
        for i in range(self.number_of_agents):
            ...
            if (
                ...
                self.loggers[i].setup_pytorch_saver(
                    self.agents[i].agent
                )  # RAD-TEAM uses own function

            # Sanity check
            if self.global_critic_flag:
                assert self.agents[i].agent.critic is self.GlobalCritic
                assert (
                    self.agents[i].GlobalCriticOptimizer is self.GlobalCriticOptimizer
                )
            elif self.actor_critic_architecture == "cnn":
                assert self.agents[i].agent.critic is not self.GlobalCritic
                if i > 0:
                    assert (
                        self.agents[i].agent.critic
                        is not self.agents[i - 1].agent.critic
                    )
                    assert (
                        not self.agents[i].GlobalCriticOptimizer
                        and not self.GlobalCriticOptimizer
                    )
        ...
        for agent in self.agents.values():
            ...
            if (
                not self.actor_critic_architecture == "rnn"
                and not self.actor_critic_architecture == "mlp"
            ):
    ...
    def train(self) -> None:
        ...
        source_coordinates: npt.NDArray = np.array(
            self.env.src_coords, dtype="float32"
        )  # Target for later NN update after episode concludes
        episode_return: Dict[int, float] = {id: 0.0 for id in self.agents}
        steps_in_episode: int = 0

        # Prepare epoch variables
        out_of_bounds_count: Dict[int, int] = {
            id: 0 for id in self.agents
        }  # Out of Bounds counter for the epoch (not the episode)
        ...
        for epoch in range(self.total_epochs):
            ...
            for steps_in_epoch in range(self.steps_per_epoch):
                ...
                if (
                    ...
                    for id in self.agents:
                        observations[id][0] = self.stat_buffers[id].standardize(
                            observations[id][0]
                        )
                ...
                next_observations, rewards, terminals, infos = self.env.step(
                ...
                for id in infos:
                    if (
                        "out_of_bounds" in infos[id]
                        and infos[id]["out_of_bounds"] == True
                    ):
                ...
                epoch_ended: bool = steps_in_epoch == (
                    self.steps_per_epoch - 1
                )  # Max steps per epoch reached
                episode_reset_next_step: bool = (
                    episode_over or epoch_ended
                )  # Either the episode is over or the episode has been cutoff by the max steps per epoch
                ...
                assert (
                    observations is not next_observations
                ), "Previous step observation is pointing to next observation already."
                ...
                if episode_reset_next_step:
                    ...
                    # If at the end of an epoch, log epoch results and reset counters
                        ...
                        for id in self.agents:
                            self.loggers[id].store(
                                DoneCount=terminal_counter[id],
                                OutOfBound=out_of_bounds_count[id],
                            )
            ...
            for id in self.agents:
                ...
                self.loggers[id].log_tabular(
                    "loss_predictor", average_only=True
                )  # Specific to the regressive GRU
    ...
    def process_render(self, epoch_ended: bool, epoch: int) -> None:
        ...
        save_time_triggered = (
            (epoch % self.save_gif_freq == 0) if self.save_gif_freq != 0 else False
        )
