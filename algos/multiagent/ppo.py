...
class AgentPPO:
        ...
        # Train RAD-TEAM framework
            ...
            if not self.GlobalCriticOptimizer or self.id == 0:
                for _ in range(self.train_v_iters):
                    ...
                    critic_loss_results = self.compute_batched_losses_critic(
                        data=data,
    ...
    def compute_loss_pi(
        self,
        data: Dict[str, Union[torch.Tensor, List]],
        index: int,
        map_stack: List[torch.Tensor],
    ):
        """
        ...
        if (
            ...
            action_logprobs, dist_entropy = self.agent.pi.evaluate(
                map_stack, act[index]
            )
        else:
            action_logprobs, dist_entropy = self.agent.pi.get_action_information(
                map_stack, act[index]
            )
        ...
        clip_adv = (
            torch.clamp(ratio, 1 - self.clip_ratio, 1 + self.clip_ratio) * adv[index]
        )  # Objective surrogate
    ...
    def compute_batched_losses_critic(self, data, map_buffer_maps, sample):
        ...
        for index in sample:
            ...
            critic_loss_list.append(
                self.compute_loss_critic(
                    data=data, map_stack=map_buffer_maps[index], index=index
                )
            )
    ...
    def update_model(self, data: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        for _ in range(self.train_pfgru_iters):
            ...
            mpi_avg_grads(
                self.agent.model
            )  # Average gradients across the processes     # MPI
    ...
    def update_rada2c(
        ...
        # A list of all episodes, that then contain a single-element list of a tensor representing the observation. 
        ...
        sampled_episodes = [ep_form[idx] for idx in ep_select]

        # Loss storage buffer(s)
        loss_array_buffer: torch.Tensor = torch.zeros((len(sampled_episodes), 1),dtype=torch.float32) # Prefilling is fast with numpy
        loss_buffer: torch.Tensor = torch.autograd.Variable(loss_array_buffer)
        loss_storage: torch.Tensor = torch.zeros((len(sampled_episodes),4),dtype=torch.float32)
        
        for index, ep in enumerate(sampled_episodes):
    ...
    def generate_mapstacks(self):
        ...
        for step in self.ppo_buffer.full_observation_buffer:
            observation = {
                key: value for key, value in step.items() if key != "terminal"
            }
            batched_actor_mapstack, batched_critic_mapstack = self.agent.get_map_stack(
                state_observation=observation, id=self.id
            )
