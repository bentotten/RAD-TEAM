...
def multi_plot(data,smooth=None, x_axis='Epoch', save_f=False, file_name="."):
    ...
    if save_f:
        if file_name[-1] != "/":
            file_name += "/"
        fig.savefig(file_name+data['condition2']+'_results.eps')
    else:
        plt.show()
...
def compare(datasets,type_=None, smooth = None, x_axis='Epoch', save_f=False, file_name=".", sample_range: int = 100):
    ...
    #fig,axs = plt.subplots(len(lst)//div,len(lst)//(div+1),figsize=(12,8), constrained_layout=True)
    plt.rc("font", size=26)
    fig,axs = plt.subplots(div,(len(min_headers)-len(exclude))//(div+1),figsize=(50,16), constrained_layout=True)
    x = result[result.columns[result.columns == x_axis][0]].to_numpy()
    ...
    if save_f:
        if file_name[-1] != "/":
            file_name += "/"
        fig.savefig(file_name+'_results_comparison.eps')
    else:
        plt.show()   
