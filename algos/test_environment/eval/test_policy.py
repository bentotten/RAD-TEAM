...
def refresh_env(env_dict,env,n,num_obs=0):
    ...
    env.agents[0].det_coords    = env_dict[key][1].copy()
    env.intensity     = env_dict[key][2]
    env.bkg_intensity = env_dict[key][3]
    env.source        = set_vis_coord(env.source,env.src_coords)
    env.agents[0].detector      = set_vis_coord(env.agents[0].detector, env.agents[0].det_coords)
    ...
    o, _, _, _        = env.step(-1)
    o = o[0]
    env.agents[0].det_sto       = [env_dict[key][1].copy()]
    env.src_sto       = [env_dict[key][0].copy()]
    env.agents[0].meas_sto      = [o[0].copy()]
    env.prev_det_dist = env.world.shortest_path(env.source,env.agents[0].detector,env.vis_graph,EPSILON).length()
    env.iter_count    = 1
    return o, env
...
def run_policy(env, env_set, render=True, save_gif=False, save_path=None, 
    ...
    max_ep_len= 120 # Max steps per episode not set anywhere, reverting to env default
    ...
    x_est = get_action(np.append((env.agents[0].meas_sto[ep_len]),env.agents[0].det_sto[ep_len]),est=True)
    obs_std = o
    obs_std[0] = np.clip((o[0]-stat_buff.mu)/stat_buff.sig_obs,-8,8) 
    
    #Get initial FIM calculation if applicable
    est_init_bnd, x_est_init = get_action([x_est,env.agents[0].det_sto[ep_len],env.bkg_intensity,scale_mat,uni_probs[None,:],obs_std],init_est=True) 
    ...
    while mc < MC: #Perform Monte Carlo runs
        ...
        action, score, hidden = get_action([x_est,np.append(env.agents[0].meas_sto[ep_len],env.agents[0].det_sto[ep_len]),obs_std,env.bkg_intensity,scale_mat],
                                            hidden=hidden,act=True,step=ep_len)

        #Take action in environment and get new observation
        # o, r, d, _ = env.step(action)
        o, r, d, _ = env.step({0: action})
        o, r, d = o[0], r['individual_reward'][0], d[0]        
        ...
        if fish_analysis:
            ...
            R_t = get_action([x_est,env.agents[0].det_sto[ep_len],env.bkg_intensity,scale_mat],FIM=ep_len)
        ...
        if d or (ep_len == max_ep_len):
            ...
            det_ls[mc].append(np.array(env.agents[0].det_sto))
            if mc < 1:
                if d:
            ...
            if render and n==(tot_ep-1) and repl < 1: 
                ...
                seq_sto['Ep'+str(mc)+'_meas'] = env.agents[0].meas_sto
                seq_sto['Ep'+str(mc)+'_det'] = env.agents[0].det_sto
            ...
            env.epoch_end = False
            env.done = False; 
            env.agents[0].out_of_bounds = False
            env.iter_count = 0
            env.agents[0].out_of_bounds_count = 0
            r, d, ep_ret, ep_len = 0, False, 0, 0
            o, env = refresh_env(env_set,env,n,num_obs=len(env.obs_coord))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_results', type=bool, default=True, help='Save list of results across episodes and runs')
    ...
    ### No longer able to serialize swig objects (for visilibity lib)
    for i in range(0, args.episodes):
        mc_results.append(run_policy(env, env_set, args.render,args.save_gif, args.fpath, args.mc_runs, args.control,args.fisher,ac_kwargs,bp_kwargs,grad_kwargs, args.episodes, n=i))
